  ...
  return knex.schema
    ...
    .then(() => {
      return knex('default_strings')
        ...
        .update({
          message:
            '\n• Прежде чем делать что-либо, нужно зарегистрироваться, написав команду `!register` в канале {{{RegistrationChannel}}}.\n• Чтобы увидеть список уровней, перейдите на {{TeamURI}}/levels.\n• Затем вы сможете отметить пройденные вами уровни, используя команду `!clear КОД-УРО-ВНЯ` в канале {{{RegistrationChannel}}}.\n• Вы также можете написать `!login` и отмечать пройденные вами уровни непосредственно на сайте.\n• Чтобы отправить свой уровень, используйте команду `!add КОД-УРО-ВНЯ` название уровня в канале {{{LevelSubmissionChannel}}}.\n• Чтобы просмотреть полный список команд, используйте команду `!help commands`.\n🌐 `!help:lang`\n🌐 `!command:lang`',
        });
    });
};
