...
class TSDiscussChannel extends TSCommand {
    ...
    async tsexec(ts,message,{code}) {
      ...
      const { channel,created }=await ts.discussionChannel(level.code,level.status===ts.LEVEL_STATUS.PENDING ? ts.channels.levelDiscussionCategory : ts.channels.pendingReuploadCategory)
