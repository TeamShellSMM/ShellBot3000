...
class undoremovelevel extends TSCommand {
    ...
    async tsexec(ts,message,{command}) {
      ...
      if(!reason) ts.userError(ts.userError(ts.message('undoRemoveLevel.noReason')));

      const player=await ts.get_user(message);
      var level=await ts.getExistingLevel(code,true)

      if(!ts.REMOVED_LEVELS.includes(level.status)) ts.userError(ts.message('undoRemoveLevel.alreadyNotRemoved',level));

    
    
      if(!(level.creator==player.name && level.status==ts.LEVEL_STATUS.USER_REMOVED || player.is_mod)) ts.userError(ts.message('undoRemoveLevel.cant',level));

    
    
      
      await ts.db.Levels.query().patch({status:level.old_status,old_status:level.status,new_code:null}).where({code})
      ...
      undoEmbed.addField("\u200b","**Notes on level status undo** :```"+reason+"```-<@" +player.discord_id + ">");
      ...
      await this.client.channels.get(ts.channels.levelChangeNotification).send(undoEmbed);
