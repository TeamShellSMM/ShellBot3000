describe('!undoremove', function () {
  ...
  it('creator user removed', async function () {
    ...
    assert.equal(result[1],'<@256> You have undid the status change of "user removed level" by Creator ')
  ...
  it('mod user removed', async function () {
    ...
    assert.equal(result[2],'<@128> You have undid the status change of "user removed level" by Creator ')
    const level=await TEST.ts.db.Levels.query().where({code:'XXX-XXX-XX3'}).first()
    assert.equal(level.old_status,TEST.ts.LEVEL_STATUS.USER_REMOVED)
    assert.equal(level.status,TEST.ts.LEVEL_STATUS.APPROVED)
  })
  ...
  it('mod removed', async function () {
    ...
    assert.equal(result[2],'<@128> You have undid the status change of "removed level" by Creator ')
